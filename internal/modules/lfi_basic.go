...
func (m *LFIBasicContextModule) Run(ctx *core.ScanContext) ([]core.Finding, error) {
	...
	for i := 0; i < concurrency; i++ {
		go func() {
			for j := range jobs {
				...
				// Hindari duplikat request yang sama
				key := j.Target + "|" + testURL
				if _, exists := seen[key]; exists {
					continue
				}

				// Race minimal: best-effort; tidak fatal jika double-check
				seen[key] = struct{}{}
	...
	// Kumpulkan results sampai worker selesai (dengan timeout lembut via buffer)
	for {
		select {
		case f, ok := <-results:
			if !ok {
				// channel ditutup (tidak kita tutup eksplisit karena worker mati ketika jobs habis)
				goto DONE
			}
			findings = append(findings, f)
		default:
			// Jika tidak ada data baru dalam buffer dan jobs habis, keluar
			goto DONE
		}
	}
