...
{C_BRIGHT_RED}╚═════╝ {C_BRIGHT_YELLOW} ╚═════╝ {C_BRIGHT_GREEN} ╚═════╝ {C_BRIGHT_CYAN}    ╚═╝  ╚═╝{C_RESET}
    ...
    def _run_url_collection(self, active_urls_file, target, speed):
        ...
        # GAU: Loop through each domain/URL in file (gau doesn't support -l flag)
        if os.path.exists(active_urls_file):
            ...
            if unique_domains:
                gau_cmd = f"rm -f {gau_file}; touch {gau_file}; "
                for domain in list(unique_domains)[
                    :10
                ]:  # Limit to 10 domains to save time
                    gau_cmd += f"gau {domain} --subs --threads {speed} 2>/dev/null >> {gau_file}; "
                self._execute_command("gau", gau_cmd, target, timeout=1200)
